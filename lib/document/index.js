  ...
  if (responses) {
    for (let response of responses) {
      ...
      if (response[2]) {
        ...
        if (ObjectType.includes(response[2])) {
          schema.type = response[2];
        } else if (response[2].startsWith('array')) {
          recursionArray(ObjectType, definitions, routers, schema, response[2])
        } else {
      ...
      let i = 3;
      while (response[i]) {
        if (response[i].indexOf('eg:(') > -1) {
          let example = response[i].replace(_.REPLACE_EG, '');
          if(schema.$ref || response[2].startsWith('array')) {
            example = JSON.parse(example);
          }
          schema.example = example;
        }else if (response[i].indexOf('enum:') > -1) {
          const enums = response[i].replace('enum:', '').split(',');

          if (response[2].startsWith('array')) {
...
function recursionArray(ObjectType, definitions, routers, schema,  itemsType){
  schema.type = 'array';
  let items = {};
  itemsType = itemsType.substring(5, itemsType.length);
  if(itemsType.startsWith('array')){
    recursionArray(ObjectType, definitions, routers, items, itemsType);
  }else if (ObjectType.includes(itemsType)) {
...
function generateParameters(request, routers, definitions) {
  ...
  while (request[i]) {
    if (request[i].indexOf('eg:') > -1) {
      const example = request[i].replace(_.REPLACE_EG, '');
